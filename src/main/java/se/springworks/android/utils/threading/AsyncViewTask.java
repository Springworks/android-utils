...
public abstract class AsyncViewTask<SomeView extends View, Params, Progress, Result>
  ...
  /**
   * Starts execution of the task and assigns it to the specified view If the view already has a
   * task assigned to it that task is cancelled
   *
  ...
  public final AsyncTask<Params, Progress, Result> execute(SomeView view, Params... params) {
    ...
    @SuppressWarnings ("rawtypes") AsyncTask task = (AsyncTask) view.getTag(R.id.ASYNCVIEWTASKID);
  ...
  /**
   * Handles the result from the execution. This method will only handle the result if the view is
   * still assigned to this task
   */
  @Override
