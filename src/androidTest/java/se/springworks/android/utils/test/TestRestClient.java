...
public class TestRestClient extends AndroidTestCase {
  ...
  public void setUp() throws Exception {
    ...
    GrapeGuice.getInjector(this).rebind(new TestRestClientModule(getContext(),
                                                                 asyncHttp,
                                                                 syncHttp)).injectMembers(this);
  }
  ...
  public void testCaching() throws InterruptedException {
    ...
    // make sure the http layer won't answer and check that we still get the new response (eg
    // cached)
    asyncHttp.clear();
    restClient.get(url, null, mockHttpResponseHandler);
    assertEquals(expectedResponse, mockHttpResponseHandler.await(500).response);
  ...
  public void testSetBaseUrl() throws InterruptedException {
    ...
    asyncHttp.setResponse(HttpUtils.createUrlWithQueryString(url + "/some/path", params),
                          expectedResponse);
  ...
  private class TestRestClientModule extends AbstractModule {
    ...
    public TestRestClientModule(Context context,
                                MockAsyncHttpClient asyncHttpClient,
                                MockSimpleHttpClient syncHttpClient) {
    ...
    protected void configure() {
      ...
      install(new FactoryModuleBuilder().implement(IKeyValueStorage.class,
                                                   NamedSharedPreferencesStorage.class).build(
          IKeyValueStorage.NamedKeyValueStorageFactory.class));
      ...
      }).to(new TypeLiteral<DiskCache<String>>() {
      });

      bind(new TypeLiteral<ICache<String>>() {
      }).annotatedWith(Names.named("restclient")).to(new TypeLiteral<DiskCache<String>>() {
      });
